...
function enemy:on_created()
  ...
  body = self:get_map():get_entity("room9_enemy")
  ...
  for i = 1, 5 do
	...
	if petals[i].life > 0 then
	  ...
	  if remaining_petals <= 0 then
	    ...
	    self:set_attack_consequence_sprite(eye_sprite, "thrown_item", 1)
	    self:set_attack_consequence_sprite(eye_sprite, "arrow", 1)
	    self:restart()
	  end
	end
