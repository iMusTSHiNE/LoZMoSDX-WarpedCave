...
for torch in map:get_entities("light_") do
  ...
  if game:get_value("swordcaveb2hard") then
...
function map:on_update()
  ...
  if not game:get_value("swordcaveb2hard")
...
-- miniboss or boss set
  ...
  if room7_enemy_1 ~= nil then
    room7_enemy_1:set_enabled(false)
    room7_enemy_2:set_enabled(false)
...
close_door_b_sensor_3.on_activated = close_door_b_sensor_1.on_activated
...
close_door_c_sensor_3.on_activated = close_door_c_sensor_1.on_activated
...
function close_door_g_sensor_1:on_activated()
  ...
  if door_f:is_open()
      and room7_enemy_1 ~= nil then
    map:close_doors("door_g")
    map:close_doors("door_f")
    sol.audio.play_music("boss")
      room7_enemy_1:set_enabled(true)
      room7_enemy_2:set_enabled(true)
  end
end
...
close_door_i_sensor_3.on_activated = close_door_i_sensor_1.on_activated
