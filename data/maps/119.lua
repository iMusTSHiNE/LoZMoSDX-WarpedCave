...
local fighting_boss_2 = false
local fighting_boss_3 = false
local arrows_timer
...
function map:on_started(destination)
  ...
  if destination == from_ending then
	...
	map:create_pickable{
	  ...
	  treasure_variant = variant,
	  treasure_savegame_variable = "hcd10",
	  x = 420,
	  y = 157,
	  layer = 1
	}
...
function start_boss_sensor:on_activated()
  ...
  if not game:get_value("b299")
    ...
    sol.timer.start(1000, function()
      ...
      boss_3:set_enabled(true)
      hero:unfreeze()
      fighting_boss = true
      fighting_boss_2 = true
      fighting_boss_3 = true
      arrows_timer = sol.timer.start(20000, repeat_give_arrows)
    end)
  end
end
...
if boss ~= nil then
  function boss:on_dead()
    ...
    map:create_pickable{
      treasure_name = "sword",
      treasure_variant = variant,
      treasure_savegame_variable = "sword3",
      x = 440,
      y = 189,
      layer = 1
    }
...
if boss_2 ~= nil then
  function boss_2:on_dead()
    -- give Heart Container
...
if boss_3 ~= nil then
  function boss_3:on_dead()
    -- give Fairies

    map:create_pickable{
      treasure_name = "fairy",
      treasure_variant = variant,
      treasure_savegame_variable = "bossd10fairy",
      x = 460,
      y = 189,
      layer = 1
    }
